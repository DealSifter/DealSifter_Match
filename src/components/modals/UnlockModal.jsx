        ...
        {canBuyExclusive ? (
          <div style={{
            ...
            background: exclusivityStatus.kind === 'partial'
              ? 'linear-gradient(135deg, rgba(126,45,0,0.16), rgba(245,158,11,0.10))'
              : 'linear-gradient(135deg, rgba(5,70,45,0.16), rgba(20,184,166,0.10))',
            textAlign: 'left',
          }}>
          ...
          {canBuyExclusive ? (
            canExclusive ? (
              <button onClick={() => onUnlock(match, { mode: exclusivityStatus.exclusivityMode || 'total', cost: exclusiveCost })} style={{ width:"100%", padding:"15px", borderRadius:14, background: exclusivityStatus.kind === 'partial' ? 'linear-gradient(90deg, #7e2d00, #f59e0b)' : 'linear-gradient(90deg, #05462d, #14b8a6)', border:"none", color:"#fff", fontWeight:900, fontSize:15, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:10, boxShadow: `0 4px 18px ${C.alpha(exclusivityStatus.kind === 'partial' ? C.gold : C.accent, 0.28)}` }}>
                ...
                {(t.unlockWithExclusivityFor || 'Unlock with exclusivity for {count} {unit}').replace('{count}', String(exclusiveCost)).replace('{unit}', exclusiveUnit)}
              </button>
            ) : (
              <button onClick={onBuyMore} style={{ width:"100%", padding:"15px", borderRadius:14, background:C.accent, border:"none", color:"#fff", fontWeight:800, fontSize:15, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:10 }}>
                <Icon name="cart" size={18} color="#fff" /> {(t.needForExclusivity || 'Need {count} {unit} for exclusivity').replace('{count}', String(exclusiveCost)).replace('{unit}', exclusiveUnit)}
              </button>
            )
          ) : null}
