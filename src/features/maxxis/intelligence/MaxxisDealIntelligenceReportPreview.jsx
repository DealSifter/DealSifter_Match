...
function InfoCard({ title, rows, icon }) {
  return <section className="maxxis-v2-info-card"><SectionTitle icon={icon}>{title}</SectionTitle><dl>{rows.length ? rows.map(([label, value, source]) => <div key={`${label}-${value}`}><dt>{label}</dt><dd>{value}{source ? <small>{source}</small> : null}</dd></div>) : <div><dt>Status</dt><dd>UNKNOWN</dd></div>}</dl></section>;
}
...
function PropertyOverview({ schema, copy, level }) {
  ...
  return <article className="maxxis-v2-page" data-report-page="1" data-report-section="PROPERTY_OVERVIEW"><ReportHeader page={1} level={level} subtitle={level === 2 ? 'Executive Summary / Property Context' : copy.release} /><div className="maxxis-v2-page-body">
    ...
    <div className="maxxis-v2-info-grid"><InfoCard icon={User} title={copy.owner} rows={[["Name", text(owner.name, copy.unknown)], ["Type", text(owner.type, copy.unknown)], ["Status", text(owner.status, copy.unknown)], ["Allowed contacts", text(contacts, copy.unknown)]]} /><InfoCard icon={Home} title={copy.characteristics} rows={[["Title", text(property.title, copy.unknown)], ["Price", money(property.price, copy.unknown)], ["Strategy", text(property.objective, copy.unknown)], ["Cap rate", property.capRate === null || property.capRate === undefined ? copy.unknown : percent(property.capRate, copy.unknown)], ["Beds / Baths", `${text(property.beds, copy.unknown)} / ${text(property.baths, copy.unknown)}`], ["Living area", text(property.sqft, copy.unknown)], ["Rehab", money(property.rehab, copy.unknown)]]} /><InfoCard icon={MapPin} title={copy.land} rows={[["Location", text(location, copy.unknown)], ["Lot size", text(property.lot, copy.unknown)], ["Source", text(property.source, copy.unknown)], ["Portfolio", text(property.portfolio, copy.unknown)], ["Labels", text(list(property.labels).join(', '), copy.unknown)]]} /></div>
...
function ScenarioCards({ title, values, formatter, copy }) {
  return <section className="maxxis-v2-kpi-block"><SectionTitle icon={TrendingUp}>{title}</SectionTitle><div>{list(values).map((item) => <span key={item.scenario}><small>{item.scenario}</small><b>{formatter(item.value, copy.unknown)}</b></span>)}</div></section>;
}
...
function MaxxisAnalysisPage({ schema, copy, page = 6, level = 3 }) {
  ...
  return <article className="maxxis-v2-page" data-report-page={page} data-report-section="MAXXIS_AI_ANALYSIS"><ReportHeader page={page} level={level} subtitle={copy.powered} /><div className="maxxis-v2-page-body"><section className="maxxis-v2-ai-summary"><SectionTitle icon={Brain}>MAXXIS EXECUTIVE SUMMARY</SectionTitle>{summaryLines.length ? summaryLines.map((line) => <span key={line}>{line}</span>) : <strong>{summary}</strong>}</section><div className="maxxis-v2-two-col"><InfoCard icon={Target} title={copy.conclusion} rows={[["Evidence", `Based on available evidence: ${schema.sections.propertyEvidence.sourceType}`], ["Profile fit", text(available(schema.sections.investmentProfile)?.score, copy.unknown)], ["Valuation", text(available(schema.sections.valuationEvidence)?.status, copy.unknown)]]} /><InfoCard icon={BarChart3} title={copy.topics} rows={risks.map((risk, index) => [`${index + 1}`, `${risk.category}: ${risk.reason || risk.explanation}`])} /></div><InfoCard icon={AlertTriangle} title={copy.questions} rows={checks.slice(0, 4).map((item, index) => [`${index + 1}`, item.endsWith('?') ? item : `${item.replace(/[.]$/, '')}?`])} /><InfoCard icon={ListChecks} title={copy.actions} rows={checks.map((item, index) => [`${index + 1}`, item])} /><p className="maxxis-v2-disclaimer">{copy.disclaimer}</p></div></article>;
}
