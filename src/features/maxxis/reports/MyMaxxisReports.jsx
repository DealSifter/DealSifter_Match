...
export function MyMaxxisReports({reports=[],language='en',onView,onPdf,onEmail,onDelete,onClose}){
  ...
  return <section className="my-maxxis-reports" role="dialog" aria-label={t.title}><header><strong>{t.title}</strong><button type="button" onClick={onClose}>← {t.back}</button></header><div className="my-maxxis-report-filters">{[['ALL',t.all],['MAXXIS_ANALYSIS',t.analysis],['DEAL_INTELLIGENCE',t.deal]].map(([id,label])=><button type="button" className={filter===id?'is-active':''} onClick={()=>setFilter(id)} key={id}>{label}</button>)}</div><input value={query} onChange={e=>setQuery(e.target.value)} placeholder={t.search}/><div className="my-maxxis-report-list">{list.length?list.map(r=><article key={r.id}><img src={r.capability==='DEAL_INTELLIGENCE'?maxxisDealAsset:maxxisAnalysisAsset} alt=""/><div><strong>{r.capability.replaceAll('_',' ')}</strong><span>{addressOf(r)||'Property'}</span><small>{new Date(r.createdAt).toLocaleDateString()} · {r.accessSource==='ONE_TIME_UNLOCK'?(r.capability==='MAXXIS_ANALYSIS'?'3 Nuggets':'5 Nuggets'):(r.accessSource||'Included')}</small></div><div><button type="button" disabled={Boolean(busyAction)} onClick={()=>onView?.(r)}>{t.view}</button><button type="button" disabled={Boolean(busyAction)} onClick={()=>run('pdf',r,onPdf)} aria-busy={busyAction===`pdf:${r.id}`}>{t.pdf}</button><button type="button" disabled={Boolean(busyAction)} onClick={()=>run('email',r,onEmail)} aria-busy={busyAction===`email:${r.id}`}>{t.email}</button><button type="button" disabled={Boolean(busyAction)} onClick={()=>{if(window.confirm(t.confirm))onDelete?.(r)}} aria-label={t.remove}>⋮</button></div></article>):<p>{t.empty}</p>}</div></section>;
}
