...
describe('canonical report journey contract',()=>{
  ...
  it('uses deterministic PDF page dimensions independent from viewport',()=>{expect(portfolio).toContain("new jsPDF({ unit: 'pt', format: 'a4' })");expect(portfolio).toContain('const topMargin = 28');expect(portfolio).not.toMatch(/generateReleasePdf[\s\S]{0,500}innerWidth/)});
