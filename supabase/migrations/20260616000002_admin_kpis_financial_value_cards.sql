...
begin
  ...
  select jsonb_build_object(
    ...
    'packPurchasesCompleted', (select count(*) from public.nugget_purchases where status = 'completed'),
    ...
    'exclusiveContactsToday', (select count(*) from public.property_unlocks where mode in ('total','partial') and created_at >= now() - interval '1 day'),
    'exclusiveContactsTotal', (select count(*) from public.property_unlocks where mode in ('total','partial')),
    'exclusiveContactsNuggetsSpent', coalesce((select sum(exclusivity_cost) from public.property_unlocks where mode in ('total','partial')), 0),
    'exclusiveContactsNuggetsSpentToday', coalesce((select sum(exclusivity_cost) from public.property_unlocks where mode in ('total','partial') and created_at >= now() - interval '1 day'), 0),
    'highlightsActive', (select count(*) from public.card_spotlights where starts_at <= now() and expires_at > now()),
    'highlightsPurchasedToday', (select count(*) from public.card_spotlights where created_at >= now() - interval '1 day'),
    'highlightsTotal', (select count(*) from public.card_spotlights),
    'highlightsNuggetsSpent', coalesce((select sum(nuggets_spent) from public.card_spotlights), 0),
    'highlightsNuggetsSpentToday', coalesce((select sum(nuggets_spent) from public.card_spotlights where created_at >= now() - interval '1 day'), 0),
    ...
    'series', jsonb_build_object(
      ...
      'highlights', (
        select jsonb_agg(jsonb_build_object('label', to_char(day_start, 'MM/DD'), 'value', (
          select coalesce(sum(cs.nuggets_spent), 0) from public.card_spotlights cs where cs.created_at >= day_start and cs.created_at < (day_start + interval '1 day')
        )) order by day_start)
        from generate_series(v_day_start::timestamp, now()::date::timestamp, interval '1 day') day_start
      ),
      'exclusive-contacts', (
        select jsonb_agg(jsonb_build_object('label', to_char(day_start, 'MM/DD'), 'value', (
          select coalesce(sum(pu.exclusivity_cost), 0) from public.property_unlocks pu where pu.mode in ('total','partial') and pu.created_at >= day_start and pu.created_at < (day_start + interval '1 day')
        )) order by day_start)
        from generate_series(v_day_start::timestamp, now()::date::timestamp, interval '1 day') day_start
      ),
      'stripe-issues', (
