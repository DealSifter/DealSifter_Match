-- Closed-sale evidence adds one cached bulk search operation. No UI, entitlement, Nugget, or Stripe changes.
...
alter table public.property_intelligence_cache
  ...
  check (data_type in ('property_record', 'property_value_avm', 'property_sold_record_pool'));
...
begin
  if p_provider <> 'rentcast'
     or p_data_type not in ('property_record', 'property_value_avm', 'property_sold_record_pool')
  ...
  on conflict (property_id, provider, data_type) do update
    set provider_property_id = excluded.provider_property_id, schema_version = excluded.schema_version,
...
alter table public.external_provider_usage
  ...
  check (operation in ('property_lookup', 'property_value_avm', 'property_sold_search'));

create or replace function public.ds_reserve_external_provider_usage(
  p_provider text, p_operation text, p_property_id uuid default null,
  p_user_id uuid default null, p_hard_limit integer default 45
)
returns uuid language plpgsql security definer set search_path = public, pg_temp as $$
declare v_usage_id uuid; v_consumed integer; v_month_key integer;
begin
  if p_provider <> 'rentcast'
     or p_operation not in ('property_lookup', 'property_value_avm', 'property_sold_search') then
  ...
  select count(*)::integer into v_consumed from public.external_provider_usage
