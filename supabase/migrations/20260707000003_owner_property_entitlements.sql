-- Owner/property entitlement correction.
-- A direct owner/contact unlock must expose every active published property
-- from that owner as unlocked in the canonical RPC payload. Property unlocks
-- remain scoped to the single property id.
...
begin
  ...
  profile_source as (
    select
      ...
      so.has_contact_access as can_show_sensitive_contact,
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'unlocked_property_ids', case
          when oc.unlock_scope in ('contact', 'reciprocal') then coalesce((
            select jsonb_agg(distinct p.id)
            from public.properties p
            where p.owner_id = oc.owner_id
              and coalesce(p.publish_to_showcase, true) = true
              and coalesce(p.is_active, true) = true
          ), '[]'::jsonb)
          else oc.unlocked_property_ids
        end,
