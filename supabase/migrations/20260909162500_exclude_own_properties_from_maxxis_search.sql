-- Maxxis Deal AI suggestions must never recommend the authenticated user's own
-- property cards as opportunities.
...
declare
  v_sql text := $query$
    ...
    where (
      ...
      and (auth.uid() is null or p.owner_id is distinct from auth.uid())
...
comment on function public.ds_search_public_properties(text[], text, text, text, numeric, numeric, integer, integer, text, uuid[], integer) is
  'Authenticated filtered discovery for active, published, open properties. Returns only sanitized Maxxis Deal AI search DTO, excludes demo mock source, and excludes the authenticated user own cards.';
