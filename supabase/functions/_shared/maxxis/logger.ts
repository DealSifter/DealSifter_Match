...
export function logMaxxisEvent(event: string, details: Record<string, unknown>) {
  ...
  return logOperationalEvent({
    ...
    metrics: {
      ...
      address_region: details.address_region,
      requested_capability: details.requested_capability,
      report_type: details.report_type,
      authenticated: details.authenticated,
      plan: details.plan,
      capability_included: details.capability_included,
      existing_unlock: details.existing_unlock,
      effective_entitlement: details.effective_entitlement,
      property_context_loaded: details.property_context_loaded,
      property_evidence_cache: details.property_evidence_cache,
      sold_evidence_cache: details.sold_evidence_cache,
      valuation_evidence_cache: details.valuation_evidence_cache,
      provider_enabled: details.provider_enabled,
      provider_allowed: details.provider_allowed,
      provider_attempted: details.provider_attempted,
      provider_result: details.provider_result,
      address_validation: details.address_validation,
      candidate_count: Number(details.candidate_count || 0),
      structural_candidate_count: Number(details.structural_candidate_count || 0),
      usable_candidate_count: Number(details.usable_candidate_count || 0),
      arv_status: details.arv_status,
      analysis_context_created: details.analysis_context_created,
